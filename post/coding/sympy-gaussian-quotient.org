...
2. as a note on how to use SymPy.

There's another blog post which shows that the ratio of two Gaussian /densities/ still being Gaussian, which is not to be confused with.
...
#+end_src
but the expression does not seem to be even if \(\mu_2 = 0\), although its graph seems symmetric (why?):
#+begin_src python
