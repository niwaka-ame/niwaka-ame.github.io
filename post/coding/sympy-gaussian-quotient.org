...
To prove \(\int_{-\infty}^{+ \infty} r p(r)\) diverge, we only need to prove \(\int_0^{+\infty} r p(r)\) diverge.
...
& \equiv g(r) \nonumber
\end{align}
and \(\int_0^{+\infty} g(r) dr\) diverges, so \(\int_0^{ +\infty} r p(r) dr\) diverges, by the [[https://math.libretexts.org/Bookshelves/Calculus/Calculus_3e_(Apex)/06%3A_Techniques_of_Integration/6.08%3A_Improper_Integration][direct comparison test for improper integrals]]. \(\blacksquare\)
